...
#SSA data
#forcing_data <- read.csv("example_simulations/data/afsis_ref_updated6.csv", as.is=T)
...
MIMruns <- forcing_data2 %>% split(1:nrow(forcing_data2)) %>% map(~ MIMICS_SS(df=.))
...
#MIMICS_ss_AllSites2$LIG_N <- MIMICS_ss_AllSites2$LIG_N...127
...
hist(MIMICS_ss_AllSites2$THETA_LIQ)

#Compare pools sizes and variation
MIM_ss_long <- MIMICS_ss_AllSites %>% select(SITE,LITm, LITs, MICr, MICk, SOMa, SOMc, SOMp) %>% pivot_longer(cols = 2:8, names_to = 'Pool', values_to = 'C_content')
...
ggplot(MIMICS_ss_AllSites) + geom_point(aes(x=CLAY, y=SOMp, colour = LIG_N), size=4) +ylab("SOMp 0-30cm (mgC/cm3)")+ xlab("Clay") +  theme_bw(base_size = 14)
