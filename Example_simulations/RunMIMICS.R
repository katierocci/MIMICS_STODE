...
forcing_data <- forcing_data %>% filter(Depth=='Topsoil')
...
ggplot(MIM_ss_long) + geom_boxplot(aes(x=Pool, y=C_content, fill=Pool, group = Pool)) +ylab("Pool C content 0-20cm (mgC/cm3")+ xlab("Pool") + 
  scale_fill_manual(values=safe_colorblind_palette, guide="none") + theme_bw(base_size = 14)

#compare inputs and outputs
ggplot(MIMICS_ss_AllSites2) + geom_point(aes(x=CLAY, y=SOMp, colour = LIG_N), size=4) +ylab("SOMp 0-20cm (mgC/cm3)")+ xlab("Clay") +  theme_bw(base_size = 14)
