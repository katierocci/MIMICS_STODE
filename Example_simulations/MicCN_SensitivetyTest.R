...
#######
#C:N applied to be average out for copios and oligos
#######

#setup for dataframe and index collection
MicCN_sens2 <- data.frame()
CN_tested2 <- numeric()
...
#   CN_s        <<- (df$CN-CN_m*fMET)/(1-fMET)
  ...
  MicCN_sens2 <- rbind(MicCN_sens2, output)
  
  
}

SensTest2 <- cbind(MicCN_sens2,MicCN2)%>% mutate(SoilC_mgCcm3=SOM_1+SOM_2+SOM_3, SoilN_mgCcm3=SOM_1_N+SOM_2_N+SOM_3_N, SoilCN=SoilC_mgCcm3/SoilN_mgCcm3)
write.csv(SensTest2, 'MIMICS_MicCNSens_difCO_102425.csv')

#soil C
ggplot(SensTest2) + geom_point(aes(x=MicCN2, y=(SOM_1+SOM_2+SOM_3)), size=4) + theme_bw(base_size = 16) + ylab("Soil C in all three pools")
#soil N
ggplot(SensTest2) + geom_point(aes(x=MicCN2, y=(SOM_1_N+SOM_2_N+SOM_3_N)), size=4) + theme_bw(base_size = 16) + ylab("Soil N in all three pools")
#soil C:N
ggplot(SensTest2) + geom_point(aes(x=MicCN2, y=((SOM_1+SOM_2+SOM_3)/(SOM_1_N+SOM_2_N+SOM_3_N))), size=4) + theme_bw(base_size = 16)+ ylab("Soil C:N")
...
#########
#C:N applied equally to copios and oligos
##########

#setup for dataframe and index collection
MicCN_sens <- data.frame()
CN_tested <- numeric()

#average microbial C:N
MicCN = c(5,10,15,20,25,30,35,40,45,50)
...
for (i in MicCN) {
  ...
  output <- t(as.data.frame(test[[1]]))
  MicCN_sens <- rbind(MicCN_sens, output)
  
  
}

SensTest <- cbind(MicCN_sens,MicCN) %>% mutate(SoilC_mgCcm3=SOM_1+SOM_2+SOM_3, SoilN_mgCcm3=SOM_1_N+SOM_2_N+SOM_3_N, SoilCN=SoilC_mgCcm3/SoilN_mgCcm3)
write.csv(SensTest, 'MIMICS_MicCNSens_sameCO_102425.csv')

#soil C
ggplot(SensTest) + geom_point(aes(x=MicCN, y=(SOM_1+SOM_2+SOM_3)), size=4) + theme_bw(base_size = 16) + ylab("Soil C in all three pools")
#soil N
ggplot(SensTest) + geom_point(aes(x=MicCN, y=(SOM_1_N+SOM_2_N+SOM_3_N)), size=4) + theme_bw(base_size = 16) + ylab("Soil N in all three pools")
#soil C:N
ggplot(SensTest) + geom_point(aes(x=MicCN, y=((SOM_1+SOM_2+SOM_3)/(SOM_1_N+SOM_2_N+SOM_3_N))), size=4) + theme_bw(base_size = 16)+ ylab("Soil C:N")
