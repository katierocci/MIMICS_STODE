...
MIMrepeat <- function(forcing_df, litBAG, dailyInput, rparams) { #need to remove litBAG if you want same litter at all sites rather than site specific litter
  ...
  # vMOD <<- c(vMOD_default[1], vMOD_default[2], vMOD_default[3], vMOD_default[4] * rparams$vMOD_x[1], vMOD_default[5] * rparams$vMOD_x[1], vMOD_default[6] * rparams$vMOD_x[1]) #for just r or K
  ...
  #fmet_p <<- c(fmet_p_default[1], fmet_p_default[2], fmet_p_default[3] * rparams$fM_x[1])
 ...
 for (site in Mic_sites) {
  BAGS_input <- filter(litBAG, Site == site)
